...
Node* decrementHelp(Node *root); // helpter to recusivly travers the tree

Node* decrementHelp(Node *root){
  if (root==nullptr) {
    return nullptr; //base case acts as an interupt to prevent segfult in next if statment/
  }
  ...
  root->left = decrementHelp(root->left); // reassigns the child as it goes in case it needs to delete.
...
void BST::decrement(){
    //begins recusive function and reassigns root at end.
    root = decrementHelp(root);
}
